...
Other aspects of `mobx-state-tree`'s API are supported as well.

#### API coverage

`mobx-quick-tree` supports the same helper functions as `mobx-quick-tree`, though some don't make sense to call on read only instances.

Functions like `getSnapshot`, `resolve`, `isStateTreeNode`, `isAlive` etc all work identically to MST on observable instances.

Functions that change data, like `applyPatch` or `applySnapshot` will work as documented in MST against observable instances, but will throw errors when run against read-only instances created with `.createReadOnly` or the ES6 Class Model API.

`mobx-state-tree` functionality like middleware, patches, and listeners like `onPatch` works as it does in `mobx-state-tree` with observable instances, and doesn't work at all against readonly instances.
...
`mobx-quick-tree` does _not_ support mixing types or instances defined with `mobx-state-tree`. `mobx-quick-tree` can co-exist in the same process, but will error if mixed in the same tree with `mobx-state-tree`.
...
const readOnlyInstance = new Todo({ name: "Hello read only", done: false }, undefined, true);
readOnlyInstance.setDone(true); // will throw an error, the instance is readonly
...
const readOnlyInstance = new Car({ make: "Toyota", model: "Prius", year: 2008 }, undefined, true);
...
const car = new Car({ make: "Toyota", model: "Prius", year: 2008 });
// run the view
...
`mobx-state-tree` and `mobx-quick-tree` both support snapshotting the rich instances defined in JS land using the `getSnapshot` function, and both conform to the same set of rules. Snapshots are useful for persisting data from one place to another, and for later re-creating instances that match with `applySnapshot`, `.create`, `.createReadOnly`, or `new ClassModel()`.
