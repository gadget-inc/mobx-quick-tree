        ...
        if (options?.dispatcher) {
            ...
            dispatcher = (snapshot) => {
                ...
                if (!discriminatorValue) {
                    throw new TypeError(`Missing discriminator property ${options.discriminator} on incoming snapshot ${JSON.stringify(snapshot)}`);
                }
                const type = discriminatorToTypeMap[discriminatorValue];
                if (!type) {
