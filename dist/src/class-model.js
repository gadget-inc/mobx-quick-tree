...
function register(object, tags, name) {
    ...
    if (klass.snapshottedViews.length > 0) {
        mstType = mstType
            .props({ __snapshottedViewsEpoch: mobx_state_tree_1.types.optional(mobx_state_tree_1.types.frozen(), 0) })
            .actions((self) => ({ __incrementSnapshottedViewsEpoch: () => self.__snapshottedViewsEpoch++ }))
            .actions((self) => {
            const hook = (0, mobx_state_tree_1.isRoot)(self) ? "afterCreate" : "afterAttach";
            return {
                [hook]() {
                    (0, mobx_1.reaction)(() => klass.snapshottedViews.map((sv) => (sv.options.createSnapshot ?? snapshot_1.getSnapshot)(self[sv.property])), () => self.__incrementSnapshottedViewsEpoch(), { equals: mobx_1.comparer.structural });
                },
            };
        });
    }
