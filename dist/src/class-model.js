...
function register(object, tags, name) {
    ...
    if (klass.snapshottedViews.length > 0) {
        mstType = mstType
            .props({ __snapshotEpoch: mobx_state_tree_1.types.frozen() })
            .actions((self) => ({
            __incrementSnapshotEpoch() {
                self.__snapshotEpoch = (self.__snapshotEpoch || 0) + 1;
            },
        }))
            .actions((self) => {
            ...
            return {
                [hook]() {
                    (0, mobx_1.reaction)(() => klass.snapshottedViews.map((sv) => {
                        let value = self[sv.property];
                        if (sv.options.createSnapshot) {
                            value = sv.options.createSnapshot(value);
                        }
                        return value;
                    }), () => {
                        self.__incrementSnapshotEpoch();
                    }, {
